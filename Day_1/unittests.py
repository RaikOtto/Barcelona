...
def test_folder_path(cifar10_path):
    assert cifar10_path is not None,\
        'Cifar-10 data folder not set.'
    assert cifar10_path[-1] != '/',\
        'The "/" shouldn\'t be added to the end of the path.'
    assert os.path.exists(cifar10_path),\
        'Path not found.'
    assert os.path.isdir(cifar10_path),\
        '{} is not a folder.'.format(os.path.basename(cifar10_path))

    train_files = [cifar10_path + '/data_batch_' + str(batch_id) for batch_id in range(1, 6)]
    other_files = [cifar10_path + '/batches.meta', cifar10_path + '/test_batch']
